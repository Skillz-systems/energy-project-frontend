...
export const DropDown = (props: DropDownType) => {
  ...
  const { items, onClickLink, buttonImgStyle, dropDownContainerStyle } = props;
  ...
  return (
    <>
      <img
        src={drop}
        alt="DropdownIcon"
        className={`w-4 h-4 cursor-pointer ${buttonImgStyle}`}
        onClick={() => setIsOpen(true)}
      />
      <div className="relative">
        <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
          <ul
            className={`${dropDownContainerStyle} absolute top-[20px] right-0 z-50 flex flex-col gap-1 p-2 bg-white border-[0.6px] border-strokeGreyThree rounded-[20px] shadow-lg w-[168px]`}
          >
